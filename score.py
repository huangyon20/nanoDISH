...
def transPredictedEventToMatrix(TestwithPred,poslist): #SVM步骤中产生的TestwithPred文件含有predict的信号值，1和-1，可以将这些值作为表达矩阵，用于reads按照结构的聚类
    """
    TestwithPred                -- Test event file with the prediction labels. Generated from SVM.py
...
def Normal_SHAPE(inlist,windowsize=200,step=5): #采用icSHAPE-pipe的策略，在Windows内，top5%为1，最低5%为0
    ...
    while start+windowsize< len(inlist):
        ...
        s95=stats.scoreatpercentile(sublist, 95)
        s5=stats.scoreatpercentile(sublist, 5)
    ...
    s95=stats.scoreatpercentile(endsublist, 95)
    s5=stats.scoreatpercentile(endsublist, 5)
...
def plotPCA(pcadata,clust_ids,cluser_ratio,outfile):
    """
...
def args():
    ...
    parser.add_argument("-r", "--reference",   type=str, required=True,      help='The reference fasta file.')
